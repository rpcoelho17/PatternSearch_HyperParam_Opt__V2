"""
California Housing - 4-way comparison: default ladder [0.5%, 1%, 10%, 100%]
with n_starts=3 on both BayesHalvingSearchCV and PatternSearchCV
(eager/patient). Optuna GPSampler result is reused unchanged since neither
the ladder nor n_starts applies to it (every Optuna trial always evaluates
100% of the data with no multi-start concept). Not logged in EXPERIMENTS.md
per explicit instruction -- script kept for reference/rerun only.
"""
import json
import time
...
# --- BayesHalvingSearchCV, n_starts=3, new ladder ---
search_bhs = BayesHalvingSearchCV(
    make_clf(), param_grid, scoring="neg_mean_absolute_error", cv=cv,
    n_jobs=-1, subsample="random", data_zones=ZONES, n_starts=3,
...
# --- PatternSearchCV, eager, n_starts=3, new ladder ---
...
# --- PatternSearchCV, patient, n_starts=3, new ladder ---
...
with open("california_housing_4way_defaultladder_nstarts3_results.json", "w") as f:
